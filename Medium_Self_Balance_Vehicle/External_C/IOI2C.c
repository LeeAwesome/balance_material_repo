...
{	
  ...
  GPIO_InitStructure.Pin = GPIO_PIN_8|GPIO_PIN_9;	//端口配置
  GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_PP;      //推挽输出
  GPIO_InitStructure.Speed = GPIO_SPEED_FREQ_MEDIUM;     //50M
...
unsigned char I2C_ReadOneByte(unsigned char I2C_Addr,unsigned char addr)
{
	...
	IIC_Start();			//发送开始命令
	IIC_Send_Byte(I2C_Addr);//发送从设备地址&写命令0xD0(D==地址，0==写)
	res++;					//res++
	IIC_Wait_Ack();			//等待应答
	IIC_Send_Byte(addr); res++; //发送要读的寄存器地址
	IIC_Wait_Ack();	  		//等待应答
	//IIC_Stop();//产生一个停止条件	
	IIC_Start();			//发送开始命令
	IIC_Send_Byte(I2C_Addr+1); res++;//发送从设备地址&读取命令0xD1(D==地址，1==写)  
	IIC_Wait_Ack();		  	//等待应答
	res=IIC_Read_Byte(0);	//读取8位数据
    IIC_Stop();//产生一个停止条件
